...
async def root():
    return {
        "message": "Welcome to RAG Chatbot Documents API with Authentication",
        "endpoints": {
            "upload": "/upload/ - Upload a PDF document (requires auth)",
            "ask": "/ask/ - Ask a question about uploaded documents (requires auth)",
...
async def upload_file(file: UploadFile = File(...), current_user: User = Depends(get_current_active_user)):
    ...
    return {"message": "Document loaded and processed", "file": file.filename, "uploaded_by": current_user.username}

@app.post("/ask/")
async def ask_question(question: str = Form(...), current_user: User = Depends(get_current_active_user)):
    qa = get_qa_system()
    if not qa.qa_chain:
    ...
    return {"question": question, "answer": answer, "asked_by": current_user.username}
